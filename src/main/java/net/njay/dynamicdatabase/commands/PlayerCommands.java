...
import net.njay.dynamicdatabase.util.PrettyPaginatedResult;
import net.njay.dynamicdatabase.util.StringUtils;
                ...
                if (args.hasFlag('a')) {
                    ...
                    for (ServerDocument document : servers) {
                        if (document.equals(DynamicDatabasePlugin.getInstance().getCurrentServerDocument())) {
                            ...
                            if (document.getOnlineStaff() == null || document.getOnlineStaff().isEmpty()) {
                                message.append(ChatColor.RED).append("No staff online!");
                    ...
                    if (serverDocument == null) {
                        sender.sendMessage(ChatColor.RED + "Server not found!" + ChatColor.AQUA + "Use " + ChatColor.GOLD + "/servers " + ChatColor.AQUA + "for a list of all servers.");
                        return;
                    }
                    ...
                    if (serverDocument.getOnlineStaff() == null || serverDocument.getOnlineStaff().isEmpty()) {
                        message.append(ChatColor.RED).append("No staff online!");
                        sender.sendMessage(message.toString());
                        return;
                    }
                    ...
                    if (serverDocument.getOnlineStaff() == null || serverDocument.getOnlineStaff().isEmpty()) {
                        message.append("\n").append(ChatColor.RED).append("No staff online!");
                        sender.sendMessage(message.toString());
                        return;
                    }
                    List<String> players = Lists.newArrayList();
            ...
            public void run() {
                ...
                try {
                    new PrettyPaginatedResult<ServerDocument>("All Servers") {
                        @Override
                        public String format(ServerDocument entry, int index) {
                            ...
                            if (entry.getOnlineStaff() == null || entry.getOnlinePlayers().isEmpty())
                                message.append(ChatColor.AQUA).append("0").append(" ");
