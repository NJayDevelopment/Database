...
/**
 * Direct port of the JavaScript LiquidMetal. https://nsakt.com/rmm5t/liquidmetal
 *
 * @author Steve Anton
 */
public class LiquidMetal {
    ...
    public static String prettifyCol(Collection<?> col) {
        ...
        for (int i = 0; i < col.size(); i++)
            if (i != 0 && i == col.size() - 1)
                sb.append(" and ").append(col.toArray()[i]);
            else if (i != 0)
                sb.append(", ").append(col.toArray()[i]);
            else
     ...
     * @author Altitude MapDev Development Team
     */
